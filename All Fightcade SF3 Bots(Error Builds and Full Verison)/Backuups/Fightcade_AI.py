...
class GhostAI:
    ...
    def __init__(self, character: str = "ken", opponent: str = "ryu"):
        ...
        self.save_dir = Path("ghost_data") / self.matchup
    ...
    def learn_from_recording(self, recorded_actions: List[RecordedAction]):
        ...
        print(f"✓ Learned {len(self.patterns)} unique situations for {self.matchup}")
    ...
    def save_patterns(self, name: str = None):
        if name is None:
            name = f"{self.matchup}_ghost_{int(time.time())}"
        filepath = self.save_dir / f"{name}.pkl"
        data = {'patterns': dict(self.patterns), 'counts': dict(self.situation_counts), 'character': self.character, 'opponent': self.opponent}
        with open(filepath, 'wb') as f:
            pickle.dump(data, f)
        print(f"💾 Saved matchup data to {filepath.name}")
    ...
    def list_ghosts(self):
        ...
        if not files:
            print(f"📁 No saved ghosts for {self.matchup}")
            return []
...
class LinuxInputExecutor(InputExecutor):
    ...
    def _find_3rd_strike_window(self):
        try:
            result = subprocess.run(["xdotool", "search", "--name", "Fightcade"], capture_output=True, text=True, check=True)
            window_ids = result.stdout.strip().split('\\n')
            for wid in window_ids:
                if not wid:
                    continue
                name_result = subprocess.run(["xdotool", "getwindowname", wid], capture_output=True, text=True, check=True)
                ...
                if "3rd Strike" in window_name or "Street Fighter III" in window_name:
                    ...
                    return wid
            result = subprocess.run(["xdotool", "search", "--onlyvisible", "--name", "3rd Strike"], capture_output=True, text=True, check=True)
...
if __name__ == "__main__":
    ...
    print("="*60)
    character = input("\nYour character (e.g. ken, ryu, chunli, makoto): ").strip().lower()
    opponent = input("Opponent character: ").strip().lower()
    ...
    if choice == "1":
        ...
        ghost_ai = GhostAI(character=character, opponent=opponent)
        bot = FightingGameBot(extractor, ghost_ai)
        bot.start_recording()

    elif choice == "3":
        ghost_ai = GhostAI(character=character, opponent=opponent)
        existing = list(ghost_ai.save_dir.glob("*.pkl"))
        if existing:
            latest = max(existing, key=lambda f: f.stat().st_mtime)
            ghost_ai.load_patterns(latest.name)
            print(f"📚 Loaded {latest.name} automatically!")
        else:
            print(f"⚠️ No saved ghosts found for {ghost_ai.matchup}")
        bot = FightingGameBot(extractor, ghost_ai)
        try:
            bot.start()
        except KeyboardInterrupt:
